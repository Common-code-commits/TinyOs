...
void set_color(int *index,RGB *rgb)
{
	...
	set_palette(*index,*index,temp_rgb);
}
...
void putfonts8_asc(int x, int y, unsigned char *s)
{
	...
	/* C语言中，字符串都是以0x00结尾 */
	for (; *s != 0x00; s++) {
		putfont(x, y,hankaku + *s * PALETTE_INIT_SIZE);
		x += 8;
	}
	return;
}
...
/* 准备鼠标光标（16x16） */
{
	static char cursor[PALETTE_INIT_SIZE][PALETTE_INIT_SIZE] = {
		"**************..",
		"*OOOOOOOOOOO*...",
		"*OOOOOOOOOO*....",
		"*OOOOOOOOO*.....",
	...
	for (y = 0; y < PALETTE_INIT_SIZE; y++) {
		for (x = 0; x < PALETTE_INIT_SIZE; x++) {
			if (cursor[y][x] == '*') {
				mouse[y * PALETTE_INIT_SIZE + x] = MOUSE_BORDER_COLOR;
			}
			if (cursor[y][x] == 'O') {
				mouse[y * PALETTE_INIT_SIZE + x] = MOUSE_FILL_COLOR;
			}
			if (cursor[y][x] == '.') {
				mouse[y * PALETTE_INIT_SIZE + x] = screen_color_index;
			}
		}
	}
	return;
